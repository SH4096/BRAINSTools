...
cp tensor.drml ~/Dropbox/Downloads
cp /Users/johnsonhj/src/BT-11/BRAINSTools-build/DWIConvert/CjyxDWIProcessing.sh ~/Dropbox/Downloads

DWICONVERTBIN="/Users/johnsonhj/src/BT-11/bin/DWIConvert"
CJYXPROC=/Users/johnsonhj/src/BT-11/BRAINSTools-build/DWIConvert/CjyxDWIProcessing.sh
...
if [[ 1 -eq 1 ]]; then
  ...
  ${CJYXPROC} ${outdir}/${PFX}${dicomDir}.nhdr
  echo "### DWIConvert FSLToNrrd " >> ${GUI_VIEW_COMMANDS}
  sed  "s#DTIIMAGEHOLDER#${PFX}${dicomDir}_CjyxDTI.nrrd#g" tensor.drml \
  |sed "s#MODELNAME#${PFX}${dicomDir}#g" \
  |sed "s#DWIIMAGEHOLDER#${PFX}${dicomDir}_Baseline.nrrd#g" > ${outdir}/${PFX}${dicomDir}.drml
  echo "cd  ${outdir};  /Applications/Cjyx.app/Contents/MacOS/Cjyx  ${outdir}/${PFX}${dicomDir}.drml" >> ${GUI_VIEW_COMMANDS}
  echo "" >> ${GUI_VIEW_COMMANDS}
fi
