  ...
  const LandmarksMapType &    updated_orig_lmks)
{
  ...
  if (!this->m_atlasVolume.empty())
  {
    ...
    LandmarksMapType referenceAtlasLandmarks = ReadCjyx3toITKLmk(this->m_atlasLandmarks);
  ...
  const std::string &                                            mapID)
{
       ...
       curr_rotationAngle_index++)
  {
    ...
    if (globalImagedebugLevel > 8)
    {
      ...
      WriteITKtoCjyx3Lmk(pointsName, msp_lmks_algo_found);
...
{ // ADD MetaData for EMSP_FCSV_FILENAME
  ...
  if (!errorLmks.empty())
  {
    WriteITKtoCjyx3Lmk(resultDir + "/" + EMSP_Fiducial_file_name, errorLmks);
  }
  if (throwException)
  {
